...
In this type of model, $x$ is partitioned by $K$ points and within each space between these points (pieces), a polynomial model is fitted. Splines are a special case of piecewise degree-d polynomial, in which a constraint ensures that it is continuous. This means that at each "knot" ($K$), that define a breaks in $x$, the line remains continuous. Piecewise polynomial regression also exists, but they do not ensure this continuity.

$$
y_i = \beta_0 + \sum_{j = 1}^{K+2}\beta_jb_j(x_i) + \varepsilon_i
$$
...
-   $x_i$ represents the proportion (%) of nitrogen (N) derived from the atmosphere (Ndfa)

-   $y_i$ represents the partial N balance (fixed N - removed N by grains) in kg/ha

The research question: Which Ndfa ($y_i$) leads to a neutral N balance ($x_i = 0$)? Or: At which proportion of N fixation all the nitrogen removed by the grain comes only from the atmosphere?
...
Fit a simple intercept-slope linear model to data using maximum likelihood and calculate the value of Ndfa that is needed to achieve a neutral N balance ($\theta$).
...
-   Similar assumptions across methods
...
-   Deficient information about $\theta$ in the data (the data alone does not have enough information on this parameter).

-   Bayesian offers the opportunity to utilize prior information to constraint $\hat{\theta}$ (data + prior knowldge).

-   We treat $\theta$ as a **random variable** allowing us to choose a prior distribution for it.
...
The Bayesian approach does not automatically solve modeling problems. It allows us to introduce additional assumptions that, when appropriate, can help address those problems.
