...
# Fit the logistic and polynomial model
ml_test <- nls(p_sev ~ K/(1 + ((K-y0)/y0)*exp(-r*time)), data = a_24, start = list(K = 0.7, y0 = 0.001, r = 0.2))
mp_test <- lm(p_sev ~ poly(time, degree = 3), data = a_24)

# Make predictions
pred <- data.frame(
  time = 1:100)

pred$pred_p <- predict(mp_test, newdata = pred)
pred$pred_l <- predict(ml_test, newdata = pred)

par(mfrow = c(1, 2))

# Plot predictions for logistic
plot(a_24$time, a_24$p_sev, ylab = "Severity (prop.)", xlab = "Days", ylim = c(-0.1, 1), main = "Logistic", xlim = c(0, 100))
lines(pred$time, pred$pred_l, lwd = 3, col = "red")

# Plot predictions for polynomial
plot(a_24$time, a_24$p_sev, ylab = "Severity (prop.)", xlab = "Days", ylim = c(-0.1, 1), main = "Polynomial", xlim = c(0, 100))
lines(pred$time, pred$pred_p, lwd = 3, col = "red")
...
-   We have different approaches for choosing the mathematical model, however, once these models have biological meaning, this should be the most important information to consider.

-   Bayesian models expand what we did before by adding more/different assumptions.
...
-   Should we be using tests to select the model?
-   Should we focus on selection criteria such as AIC/BIC?
...
<center>Different techniques can help with model selection, however, for mechanistic models that have deep biological meaning, selection criteria might not make sense. We should focus on the biological meaning.</center>
:::
...
-   We need more assumptions, one of our parameters is almost non-identifiable.
...
What have we been doing so far with the logistic model?
...
-   This is not working because of weak identifiability (our data lack information in some parameters).

    -   We could simplify the model in other ways (recall: reparametrization; linearization; parameter fixation).

    -   We could collect more data/repeat the experiment.

-   But what if we can't collect more data and are not happy about the assumptions of other methods?

-   We want this logistic equation, so let's make more assumptions!
...
How the predictions (expected value) looks like with 95% Posterior Predictive Interval[^04-bayesian-nonlinear-models-1]:

[^04-bayesian-nonlinear-models-1]: Intervals:
...
-   Let's use variety a in 2023 for example.

Minimal assumptions: Likelihood approach:

$$
y_i \sim N(\frac{K}{1+(\frac{K-y_0}{y_0})e^{-rt_i}}, \sigma^2)
$$

Bayesian:

$$
y_i \sim Beta(\frac{K}{1+(\frac{K-y_0}{y_0})e^{-rt_i}}, \phi) \\[20pt] K \sim Beta(6, 2) \\[10pt] y_0 \sim Unif(0.001, 0.1) \\[10pt] r \sim Unif(0.1, 0.5) \\[10pt] \phi \sim Gamma(24, 2)
$$
...
# Fit the model
m3 <- nls(p_sev ~ K/(1+((K-y0)/y0)*exp(-r*time)), start = c(K = 0.7, r = 0.2, y0 = 0.001), data = a_23)

# Predictions with prediction interval
df_pred <- data.frame(
  time = seq(1, max(a_23$time)+20, by = 1)
)
...
plot(a_23$time, a_23$p_sev, ylab = "Severity (prop.)", xlab = "Days", main = "Likelihood approach with minimal assumptions", ylim = c(-0.5, 1.1))
...
table_data <- data.frame(
  Day = c(1, 37, 73, 1, 37, 73),
  ub = c(df_pred$pred_lb[1], df_pred$pred_lb[37], df_pred$pred_lb[73], bay_pred$y.lb[1], bay_pred$y.lb[37], bay_pred$y.lb[73]), 
  lb = c(df_pred$pred_ub[1], df_pred$pred_ub[37], df_pred$pred_ub[73], bay_pred$y.ub[1], bay_pred$y.ub[37], bay_pred$y.ub[73]),
  Estimation = c(rep("Min. Assump.", 3), rep("Bayesian", 3))
)

ggplot(table_data, aes(x = factor(Day), ymin = lb, ymax = ub, color = Estimation)) +
  geom_linerange(position = position_dodge(width = 0.4), linewidth = 1.2) +
  labs(x = "Day", y = "Uncertainty width", color = "Estimation")
```
...
A straight forward way to check if a distribution make sense for a parameter or the data generating process involves understand it's support.
...
Also, Bayesian is not doing magic, but allowing us to work with mode assumptions to constrain our parameters.
...
-   Thank you very much for you attention and for attending this workshop, this was a great opportunity for us all to learn more together! Stay tuned for the next workshop [Bayesian Modeling of Designed Experiments](https://www.k-state.edu/engagement/innovation-and-partnerships/interdisciplinary-institutes/digital-ag-advanced-analytics/about/analytics-workshops.html) on March 2, 4, and 5 (registration has already started!).

-   Please, take a time to give us a feedback about this workshop, we would love to know your thoughts: <https://kstate.qualtrics.com/jfe/form/SV_8wgktl4walXUc2q>.
